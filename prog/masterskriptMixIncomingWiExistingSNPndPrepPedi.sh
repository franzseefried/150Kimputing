...
if [ ${snpstrat} != "F" ] && [ ${snpstrat} != "S" ]; then
   echo "unbekannte SNP-Stategie ausgewaehlt: S oder F allowed aber ${snpstrat} angegeben"
   exit 1
fi
#check Server
...
##################################
if [ ${1} == "BSW" ] || [ ${1} == "HOL" ] ; then
for i in $TMP_DIR/${1}HD.bed $TMP_DIR/${1}LD.bed ; do
if test -s ${i}; then
...
exit 1
#script needs to be tested with LINKs
$BIN_DIR/linuxPLINKselectSNP.sh $1 2>&1 #> $LOG_DIR/linuxPLINKselectSNP.${1}.log
err=$(echo $?)
if [ ${err} -gt 0 ]; then
        echo "ooops Fehler 10b"
...
echo "----------------------------------------------------"
   ...
   $BIN_DIR/sendErrorMail.sh $PROG_DIR/masterskriptMixIncomingWiExistingSNPndPrepPedi.sh
   exit 1
fi
RIGHT_NOW=$(date)
