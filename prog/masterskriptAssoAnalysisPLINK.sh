...
echo "----------------------------------------------------"
##################################
echo "Run Assoc Study using PLINK for ${GWAStrait} and for $1 withour stratification correction"
$BIN_DIR/AssocAnalPLINK.sh ${1} 2>&1 
err=$(echo $?)
if [ ${err} -gt 0 ]; then
        echo "ooops Fehler 2"
        $BIN_DIR/sendErrorMail.sh $BIN_DIR/AssocAnalPLINK.sh ${1}
        exit 1
fi
echo "----------------------------------------------------"
##################################
