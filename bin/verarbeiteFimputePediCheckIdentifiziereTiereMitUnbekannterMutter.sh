...
breed=${1}
	...
	grep "No match" ${FIM_DIR}/${breed}_PEDICHECKout/parentage_test.txt |\
            awk -v T1=${TUMMY1} '{if($3 == T1) print $1" N"}' | sort -T ${SRT_DIR} -u | sort -T ${SRT_DIR} -t' ' -k1,1 > $TMP_DIR/${breed}.TiereMitUnbekannterMutter.srt
 	#mache Liste fuer Verbaende, hier mit unterscheidung ob Vater oder Mutter, neu 14.05.2014
	i=Dam
	grep "No match" ${FIM_DIR}/${breed}_PEDICHECKout/parentage_test.txt |\
           awk -v s=${i} '{if($4 == s) print}' |\
           awk -v T1=${TUMMY1} '{if($3 == T1 ) print $1,"Mutter"}' | sort -T ${SRT_DIR} -u | sort -T ${SRT_DIR} -t' ' -k1,1 > $TMP_DIR/${breed}.TiereMitUnbekannterDam.srt
