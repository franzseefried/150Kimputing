...
echo "To: ${person}
From: ${person}
Subject: Attention!!! high proportion of low-callrate samples at GenoExPSE SNPs

Dear colleague,

callrate at GenoExPSE SNPs was checked within Samples that passed quality control based on chip data.
 
A proportion of ${2} % of the processed samples has a callrate at GenoExPSE SNPs below ${3}, which is highly suspicious. 

File with calculated callrates is ${HIS_DIR}/\*.${run}.GenoExPSEsnp.SAMPLEclrt
Format: idanimal No.ofSNPsChip callrateChip CallrateISAGgenoeExPSE
