...
#erstelle Liste mit den Tieren deren Vater genotypisiert ist, da diese unten dan negativ gejoint werden
...
join -t' ' -o'1.1 1.2 2.2 2.3' -1 1 -2 1 <(sort -T ${SRT_DIR} -t' ' -k1,1 $WORK_DIR/${breed}Typisierungsstatus${run}.txt) <(awk '{ sub("\r$", ""); print }' $WORK_DIR/animal.overall.info | cut -d';' -f2,7,12 | tr ';' ' ' | sort -T ${SRT_DIR} -t' ' -k1,1) |\
           ...
           awk '{if($4 != "-") print}' > $TMP_DIR/${breed}.samplesWithGenotypedSires.toReduceFromVVcheck.lst



 join -t' ' -o'1.1 1.2 1.3' -v1 -1 1 -2 1 <(awk -v bbb=${minplausibleMVrelship} '{if($5 < bbb) print $1,$3,$5}' ${RES_DIR}/${breed}.out.AnimalVV.snp1101.${run}.txt | sort -T ${SRT_DIR} -t' ' -k1,1) <(awk '{print $1,"ok"}' $WRK_DIR/${breed}_VVOK.default.lst | sort -T ${SRT_DIR} -t' ' -k1,1 ) |\
    sort -T ${SRT_DIR} -t' ' -k1,1 |\
    join -t' ' -o'1.1 1.2 1.3' -v1 -1 1 -2 1 - <(sort -T ${SRT_DIR} -t' ' -k1,1 $TMP_DIR/${breed}.samplesWithGenotypedSires.toReduceFromVVcheck.lst) > $ZOMLD_DIR/${breed}_suspiciousVVs.${run}.csv

if ! test -s $ZOMLD_DIR/${breed}_suspiciousVVs.${run}.csv;then
echo "b b" > $ZOMLD_DIR/${breed}_suspiciousVVs.${run}.csv
fi
rm -f $TMP_DIR/${breed}.samplesWithGenotypedDams.toReduceFromVVcheck.lst
