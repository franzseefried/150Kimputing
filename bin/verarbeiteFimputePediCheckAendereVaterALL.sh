...
#anhaengen der Tiere bei denen der surrogate sire zu jung ist, geht einfach via idimputing > idimputing surrogate sire, da renumeriertes pedigree
        ...
        #    awk '{if($4 == "Sire" ) print $1,$2,$4,$10}' | sort -T ${SRT_DIR} -u | sort -T ${SRT_DIR} -t' ' -k1,1 | awk '{print $1,$2,"Sire No_Match"}'  >> $TMP_DIR/${breed}.TiereMitVATERPROBLEM_undOHNEsurrogateSIRE.srt

#ohne plausi auf gebdat via idimputing tier > idimputing sire
	grep -v "No match" ${FIM_DIR}/${breed}_PEDICHECKout/parentage_test.txt | sed 's/No match/No_Match/g' |\
            awk -v T1=${TUMMY1} '{if($2 != T1 || $3 != T1) print}' |\
            awk '{if($4 == "Sire" ) print $1,$2,$4,$10}' | sort -T ${SRT_DIR} -u | sort -T ${SRT_DIR} -t' ' -k1,1 > $TMP_DIR/${breed}.TiereMitVATERPROBLEM_ABERsurrogateSIRE.srt
