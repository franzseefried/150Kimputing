...
if [ -z $2 ]; then
   ...
   exit 1
elif [ $2 == "SINGLEGENE" ] || [ $2 == "SVM" ] || [ $2 == "HAPLOTYPE" ] || [ $2 == "GENOMEWIDE" ]; then
   echo $2 > /dev/null
   strat=${2}
else
   echo "Der Code fuer die Strategie muss SINGLEGENE SVM HAPLOTYPE GENOMEWIDE sein, ist aber ${2}"
   exit 1
fi
...
checkB=$(awk -v a=${colABAASGI} -v b=${colCSGI} -v c=${colIMPBRD} -v d=${breed} -v e=${strat} 'BEGIN{FS=";"}{if($a == e && $c ~ d) print $1}' ${REFTAB_SiTeAr} | wc -l |awk '{print $1}')
checkC=$(awk -v a=${colABAASGI} -v b=${colCSGI} -v c=${colIMPBRD} -v d=${breed} -v e=${strat} 'BEGIN{FS=";"}{if($a == e && $c ~ d) print $b}' ${REFTAB_SiTeAr} | wc -l | awk '{print $1}')

if [ ${checkB} != ${checkC} ]; then
echo "OOOPS: There are tests nominated for ${strat} which have no code for the result.. check"
echo " "
exit 1
fi

#write file
awk -v a=${colABAASGI} -v b=${colCSGI} -v c=${colIMPBRD} -v d=${breed} -v e=${strat} 'BEGIN{FS=";"}{if($a == e && $c ~ d) print $b}' ${REFTAB_SiTeAr} | tr ' ' '\n' | sort -u | tr '\n' ' ' > $TMP_DIR/${breed}.${strat}.selected
