...
library(ggplot2)
library(qqman)
#setwd("/Volumes/data_projekte/projekte/snp/ProgEvalFimpute/")
args <- commandArgs(trailingOnly=TRUE)
print(args)
#args<-c("results/150K1 results/150K2 /Volumes/data_tmp/projekte/snp/150Kimputing1/tmp/selectedMASK.animals.BSW 0")
#ags<-unlist(str_split(args," "))
ags<-args

anim<-list()
#Suche Anim files

for(i in length(ags):1)
{
if(file.exists(paste(ags[i],"_Ind_correlations.txt",sep=""))) {
  if(i==length(ags)){print("No Breed information provided")
    break} else {
    print("Some breed information is given")
  break}} else if(ags[i]!=0){
    anim[[i]]<-read.table(paste(ags[i]))
  }else{
    anim[[i]]<-"empty"
  }
...
{
ge_files[[i]]<-read.table(paste(ags[i],"_Ind_correlations.txt",sep=""))  
...
for(i in 1:length(ags))
{
  if(file.exists(paste(ags[i],"_higher_Ind_correlations.txt",sep=""))) {
  hi_files[[mi]]<-read.table(paste(ags[i],"_higher_Ind_correlations.txt",sep=""))  
...
for(i in 1:length(ags))
{
  if(file.exists(paste(ags[i],"_lower_Ind_correlations.txt",sep=""))) {
    lo_files[[mi]]<-read.table(paste(ags[i],"_lower_Ind_correlations.txt",sep=""))  
...
melted_compl<-melt(compl,id.vars=c("Inds","Scen","MAF"),measure.vars = c("Corr","Conc"))
...
for( a in 1:length(anim))
{
  ...
  melted_compl$breed[which(melted_compl$Inds%in%anim[[a]]$V1)&melted_compl$Scen==scens[n]]<-anim[[a]]$breed
  n<-n+1}
}

table(melted_compl$breed)
pdf(paste(format(Sys.time(), "%Y%m%d%H%M%S"), "ImputationAccuracyplots.pdf", sep = ""))
...
for(i in 1:length(ags))
{
  maf_ge_n<-read.table(paste(ags[i],"_MAF_correlations.txt",sep=""),sep=":",skip=1,nrows=13) 
  maf_ge[[i]]<-read.table(paste(ags[i],"_MAF_correlations.txt",sep=""),skip=15,nrows=13,as.is=TRUE)
...
for(i in 1:length(ags))
{
  if(file.exists(paste(ags[i],"_higher_MAF_correlations.txt",sep=""))){
  maf_ge_n<-read.table(paste(ags[i],"_higher_MAF_correlations.txt",sep=""),sep=":",skip=1,nrows=13) 
  maf_hi[[mi]]<-read.table(paste(ags[i],"_higher_MAF_correlations.txt",sep=""),skip=15,nrows=13,as.is=TRUE)
...
for(i in 1:length(ags))
{
  if(file.exists(paste(ags[i],"_lower_MAF_correlations.txt",sep=""))){
    maf_ge_n<-read.table(paste(ags[i],"_lower_MAF_correlations.txt",sep=""),sep=":",skip=1,nrows=13) 
    maf_lo[[mi]]<-read.table(paste(ags[i],"_lower_MAF_correlations.txt",sep=""),skip=15,nrows=13,as.is=TRUE)
...
for(i in 1:length(ags))
{
  if(file.exists(paste(ags[i],"_SNP_correlations.txt",sep=""))){
    snp_ge<-read.table(paste(ags[i],"_SNP_correlations.txt",sep=""))
    manhattan(snp_ge,chr="V1",bp="V2",p="V5",logp=FALSE,
              main=paste(tail(unlist(str_split(ags[i],"/")),n=1)),ylab="Imputation error %",
